...
const customeBoxMessage=(prevState,className,content)=>{
  return (<React.Fragment>{prevState}<br/>
  ...
  useEffect(() => {
    const loadVideo = async () => {
      ...
      hls.on(Hls.Events.MANIFEST_PARSED, function () {
        console.log(player)
        setLogger((prevState)=>{return customeBoxMessage(prevState,'logger-message-MANIFEST_PARSE',(<>{decodeString(player.current.baseURI) }<br/>{decodeString( player.current.currentSrc)}</>))});
        try {
          video.play();
        } catch (error) {
          console.log(error);
